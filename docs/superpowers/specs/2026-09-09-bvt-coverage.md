# BVT coverage: what the suites prove today and the sweep that would be complete
...
*BVT* = a build verification sweep of the user journeys, which is what a
...
| # | Journey | Unit | E2E | BVT case to add |
|---|---|---|---|---|
...
| 17 | Password auth, saved password | crypto, config | — | AUTH-2: needs a password-auth user on tompi (create `bvtuser`) |
...
| 35 | OSC title sets the tab title | vt_sequences | — | TERM-5: `printf '\e]0;BVT\a'`, tab caption reads BVT |
...
- **Per build (BVT), about 3 minutes, no AI calls:** LAUNCH-1..3, RESIZE-1, WINDOW-1..2, CLOSE-1, MENU-1, SM-1..5, HK-1..2, CONN-1..2, TERM-1..5, CLIP-1..2, TABS-1..2, LOG-1..2, SET-1..4, CLI-1, plus the existing non-AI cases and `ui_gallery`.
