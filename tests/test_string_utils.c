...
/* ---- cell_text_codepoint (hardening) ------------------------------------ */

int test_cell_text_codepoint_empty_cell_is_space(void)
{
    TEST_BEGIN();
    ASSERT_EQ(cell_text_codepoint(0), (uint32_t)' ');
    TEST_END();
}
...
{
    TEST_BEGIN();
    ASSERT_EQ(cell_text_codepoint(0x01), (uint32_t)' ');   /* SOH */
    ASSERT_EQ(cell_text_codepoint(0x07), (uint32_t)' ');   /* BEL */
    ASSERT_EQ(cell_text_codepoint(0x1B), (uint32_t)' ');   /* ESC */
    ASSERT_EQ(cell_text_codepoint(0x1F), (uint32_t)' ');
    TEST_END();
}
...
{
    TEST_BEGIN();
    ASSERT_EQ(cell_text_codepoint(0x7F), (uint32_t)' ');
    TEST_END();
}
...
{
    TEST_BEGIN();
    ASSERT_EQ(cell_text_codepoint(0x80), (uint32_t)' ');
    ASSERT_EQ(cell_text_codepoint(0x9B), (uint32_t)' ');   /* CSI (8-bit form) */
    ASSERT_EQ(cell_text_codepoint(0x9F), (uint32_t)' ');
    TEST_END();
}
...
{
    TEST_BEGIN();
    ASSERT_EQ(cell_text_codepoint((uint32_t)'A'), (uint32_t)'A');
    ASSERT_EQ(cell_text_codepoint(0x20), (uint32_t)' ');    /* plain space itself */
    ASSERT_EQ(cell_text_codepoint(0xA0), (uint32_t)0xA0);   /* NBSP: just past C1, kept */
