...
Invoke-Case "cli_no_connect_opens_idle" @{} {
    ...
    Assert-True ((Get-NutshellRegionHash -Path $p1 -Region $fullWindow) -eq (Get-NutshellRegionHash -Path $p2 -Region $fullWindow)) `
        "the window repainted over 5s with -nc -- looks like a connection attempt (a connecting-state tab pulses)"
    "no dialog and no repaint over 5s with -nc: no connection attempt"
} -ExtraArgs @("-nc")
